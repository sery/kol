...
boolean bufferoutput(string stuff)
{
  ...
  return true; //will return an error soon elsewise.
}
...
{
  int numgot = available_amount(to_item(itemname));
...
int hasHowManyOf(string [string] itemlist)
{
  ...
  {
    amount = amount + available_amount(to_item(var));
  }
  return amount;
}
...
void telescope()
{
  ...
  */
  foreach itm in Telescope1 {
    Telescope[itm] = 1;
  }
  foreach itm in Telescope2 {
    Telescope[itm] = 1;
  }
  foreach itm in Telescope3 {
    Telescope[itm] = 1;
  }
	...
	if (telescopeupgrade > 0)
	{
	  ...
	  //If we have a telescope, we can ALWAYS see the first gate.
		...
		if (telescopeupgrade >= 6)
		{
		  foreach itm in Telescope2
		  {
		    if ((Telescope2[itm] == upgrade[2]) || (Telescope2[itm] == upgrade[3]) || (Telescope2[itm] == upgrade[4]) || (Telescope2[itm] == upgrade[5]) || (Telescope2[itm] == upgrade[6]))
		    {
		      Telescope[itm] = 2;
		    } else {
		      Telescope[itm] = 3;
		  ...
		  foreach itm in Telescope2
		  {
		    //This time, we won't set Telescope[itm] = 3, because we don't know what we DONT need if we have < 6 telescope upgrades.
		    if ((Telescope2[itm] == upgrade[2]) || (Telescope2[itm] == upgrade[3]) || (Telescope2[itm] == upgrade[4]) || (Telescope2[itm] == upgrade[5]))
		    {
		      Telescope[itm] = 2;
		    }
		  }
    ...
    This is obviously just for testing.
  	...
  	foreach itm in Telescope
  	{
  	  print(to_string(itm) + " --- " + to_string(Telescope[itm]));
  	}
  	abort();
  */
}
...
void bumcheekcitys_hardcore_checklist()
{
  ...
  if (my_level >= 1)
  {
    ...
    itemlist["fat loot token"] = "You don't have any fat loot tokens.";
    if (hasHowManyOf(itemlist) < 2)
    {
      bufferoutput("You don't have at least 2 of Boris's Key, Jarlsberg's Key, Sneaky Pete's Key, and/or fat loot tokens.");
    } else {
      //Should probably test for the zap group here.
      debug("You have at least two of the Boris/Jarl/Pete's Keys or tokens.");
    }
  ...
  if (my_level >= 2)
  {
    ...
    if (my_path() != "Avatar of Boris" 
    	...
    	debug("You are a(n) "+my_path()+", you don't need no stinkin' familiars.");
    	}
  ...
  if (my_level >= 3)
  {
    ...
    if (item_amount($item[dingy dinghy]) > 0 || item_amount($item[skeletal skiff]) > 0)
    {
      debug("You have discovered the Island.");
    	...
    	if(item_amount($item[skeleton]) > 7){
    ...
    bufferoutput(hasItem("jaba–ero-flavored chewing gum", 1));
  ...
  if(my_level >= 6)
  {
    ...
    if (((my_path() != "Avatar of Boris") && (my_path() != "Zombie Slayer")	)	|| (((my_path() == "Avatar of Boris") || (my_path() == "Zombie Slayer")) && (my_level == 11) )){
    ...
    if ((item_amount($item[wet stew]) + item_amount($item[wet stunt nut stew]) + item_amount($item[Mega Gem]) == 0) || have_equipped($item[Mega Gem]))
    {
      ...
      bufferoutput(hasItem("bird rib", 1));
    	...
    	debug("If you were a regular class, you could visit Whitey's Grove, but you are a(n) "+my_path() + " and so can't visit it until level 11. (You are level "+my_level()+".)");}
    //Orc & Hippy Outfits. 
  ...
  if(my_level >= 8)
  {
    if (urlContains("questlog.php?which=2", "peace to Mt. McLargeHuge"))
    {
      debug("The Mt. McLargeHuge Quest is done.");
    } else if (urlContains("questlog.php?which=1", "really very close to figuring out what")){
      bufferoutput("You need to kill Groar.");    
    ...
    } else if (urlContains("questlog.php?which=2", "The Tr4pz0r wants you to bring him 3 chunks of goat cheese from the Goatlet.")) {
      ...
      int totalNs = item_amount($item[lowercase N]) + item_amount($item[NG]) + item_amount($item[ND]) + item_amount($item[Wand of Nagamar]);
   	...
   	if ((my_path() == "Bees Hate You")||(my_path() == "Avatar of Boris")||(my_path() == "Zombie Slayer")){
   		if (northPoleDef){
   			...
   			if (totalNs < 1) bufferoutput("You might need 1 lowercase N, but you don't have any.");
   			}
   		}  		
    else {
    	if (northPoleDef)
	        ...
	        if (totalNs < 1){
 		         bufferoutput("You only need 1 lowercase N, but you don't have any.");
   			     }
      		}
    }
    }
     ...
     if(item_amount($item[steam-powered model rocketship]) > 0)
      {
        ...
        if(item_amount($item[star hat]) == 0){
        	starsNeeded = starsNeeded + 5;
        	linesNeeded = linesNeeded + 3;
        	chartsNeeded = chartsNeeded + 1;
        	}
        	
        if(item_amount($item[Richard's Star Key]) == 0){
        	starsNeeded = starsNeeded + 8;
        	linesNeeded = linesNeeded + 7;
        	chartsNeeded = chartsNeeded + 1;
        	}
        	
        string [string] starlist;
			starlist["star sword"] = "You don't have a star sword.";
    		starlist["star crossbow"] = "You don't have a star crossbow";
    		starlist["star staff"] = "You don't have a star staff.";
    		
    		if ((my_path() == "Avatar of Boris") || (my_path() == "Way of the Surprising Fist")) {
    			debug("You don't need a star weapon.");
    			}		
    		else if (hasHowManyOf(starlist) < 1)
    			{
    			chartsNeeded = chartsNeeded + 1;
    			if( (my_class() == $class[Seal Clubber]) || ( (my_class() == $class[Turtle Tamer]) || (my_class() == $class[Zombie Master]) ) ) {
    				starsNeeded = starsNeeded + 7;
        			linesNeeded = linesNeeded + 4;
        			}
    			if( (my_class() == $class[Sauceror]) || (my_class() == $class[Pastamancer]) ) {
    				starsNeeded = starsNeeded + 6;
        			linesNeeded = linesNeeded + 5;
        			}
    			if( (my_class() == $class[Disco Bandit]) || (my_class() == $class[Accordion Thief]) ) {
    				starsNeeded = starsNeeded + 5;
        			linesNeeded = linesNeeded + 6;
        			}
    			
    			
      				bufferoutput("You need a star weapon.");
    			} else {
      			//Should probably test for the zap group here.
      			debug("You have a star weapon.");
    			}
        //bufferoutput(hasItem("star crossbow", 1));
        //bufferoutput(hasItem("star sword", 1));
       // bufferoutput(hasItem("star staff", 1));
    	bufferoutput(hasItem("star", starsNeeded));
    	bufferoutput(hasItem("line", linesNeeded));
    	bufferoutput(hasItem("star chart", chartsNeeded));
      } else if (item_amount($item[S.O.C.K.]) > 0) {
